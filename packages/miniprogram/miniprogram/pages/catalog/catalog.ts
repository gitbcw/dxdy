  ...
  async loadProducts() {
    ...
    this.setData({
      ...
      summaryCards: [
        ...
        { value: String(mappedProducts.filter((item: any) => item.lowStock).length), label: '低库存提醒', desc: '建议优先采购' },
        { value: String(mappedProducts.filter((item: any) => item.isBloodPack).length), label: isInstitution ? '预约服务' : '服务商品', desc: isInstitution ? '可直接发起预约' : '登录机构后可见更多能力' },
      ],
      quickFilters,
      activeQuickFilter,
