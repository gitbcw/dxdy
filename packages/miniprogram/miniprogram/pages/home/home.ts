...
Page({
  ...
  getDashboard(role: DemoRole, data: any) {
    if (role === 'customer_institution') {
      ...
      const returnOrder = data.allReturns.find((record: any) => record.orderId === shippingOrder?.id || record.status === 'pending_review')
      const bloodProducts = data.products.filter((product: any) => product.isBloodPack).slice(0, 3)
      ...
      return {
        ...
        summaryCards: [
          ...
          { value: String(data.allReturns.filter((record: any) => record.status !== 'completed').length), label: '售后处理中', desc: '退款、换货统一跟进' },
          { value: `¥${formatMoney(data.customerOrders.reduce((sum: number, order: any) => sum + order.pricing.actualAmount, 0))}`, label: '累计采购', desc: '机构价已生效' },
        ],
        taskCards: [
          {
            badge: '优先',
          ...
          },
          {
            ...
            meta: bloodProducts.length ? bloodProducts.map((item: any) => item.name).join(' · ') : '查看犬猫血液制品',
            action: 'blood',
            actionText: '去预约',
          },
          {
    ...
    if (role === 'salesperson') {
      ...
      return {
        ...
        boardTitle: '今天最值得跟进',
        boardMoreText: '查看客户',
        boardItems: recentCustomers.map((customer: any) => ({
          id: customer.id,
    ...
    if (role === 'clerk') {
      ...
      return {
        ...
        taskCards: [
          ...
          {
            badge: '换货',
            title: data.clerkPending.some((item: any) => item.type === 'exchange') ? '换货单需要优先处理' : '暂无换货单',
            desc: data.clerkPending.some((item: any) => item.type === 'exchange')
    ...
    const featuredProducts = data.products.slice(0, 3).map((product: any) => ({
      id: product.id,
    ...
    return {
      ...
      summaryCards: [
        ...
        { value: String(data.allReturns.filter((record: any) => record.status !== 'completed').length), label: '售后中', desc: '退款和换货进度' },
      ],
      taskCards: [
        {
          badge: '下单',
