...
const { formatMoney, getOrders, getClerkOrders, getCommissionSummary, getSalesmanCustomers, } = require('../../services/index');
Page({
    data: {
        userInfo: null,
        balance: '0.00',
    ...
    async loadUserInfo() {
        ...
        const userRole = app.globalData.userRole || 'customer_personal';
        if (!user) {
    ...
    async getRoleCopy(role, user) {
        if (role === 'salesperson') {
            ...
            return {
                ...
                focusTitle: '今天优先跟进',
                focusItems: recentCustomers.map((customer) => ({
                    id: customer.id,
        ...
        if (role === 'clerk') {
            ...
            return {
                ...
                focusTitle: '现在最急的单',
                focusItems: pending.slice(0, 3).map((order) => ({
                    id: order.id,
        ...
        const shippingOrder = orders.find((order) => order.status === 'pending_receipt');
        return {
    ...
    onFocusItemTap(e) {
        const id = e.currentTarget.dataset.id;
        const item = this.data.focusItems.find((m) => m.id === id);
        const page = this;
        if (item && page[item.tap]) {
            page[item.tap]();
        }
    ...
    onSwitchRole() {
        ...
        wx.showActionSheet({
            ...
            success: async (res) => {
                ...
                if (selected.role === this.data.currentRole)
                    return;
