...
Page({
    ...
    async onLoad(options) {
        ...
        const defaultAddressIndex = Math.max(0, addresses.findIndex((item) => item.isDefault));
        ...
        this.setData({
            product,
            addresses,
            addressOptions: addresses.map((item) => `${item.name} ${item.phone} ${item.province}${item.city}${item.district}${item.detail}`),
    ...
    async onSubmit() {
        const { product, quantity, remark, orderType, contactName, contactPhone, bookingDate, bookingLocation, } = this.data;
        const user = getApp().globalData.userInfo;
        if (!product || !user)
            return;
