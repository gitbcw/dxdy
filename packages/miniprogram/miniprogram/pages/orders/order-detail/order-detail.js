...
const { getOrders, getOrderById, updateOrderStatus, createReturn, getReturns, formatMoney, formatDateTime, } = require('../../../services/index');
const statusLabel = {
    pending_payment: '待支付',
...
Page({
    ...
    getSummaryCards(orders) {
        return [
            { value: String(orders.length), label: '全部订单', desc: '统一查看支付与售后' },
            { value: String(orders.filter((item) => item.status === 'pending_payment').length), label: '待支付', desc: '优先处理改价单' },
            { value: String(orders.filter((item) => item.returnRecord).length), label: '售后中', desc: '退款与换货进度' },
        ];
    },
    ...
    getCommissionText(order, returnRecord) {
