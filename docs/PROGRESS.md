...
P0 阶段已完成：小程序三端（客户/代理商/制单员）主要页面和 16 个云函数已部署，后台 11 个管理页面全部脱离 mock 接入 CloudBase，鉴权和审计已闭环。下一步进入 **P1：上线前安全收口 + 业务补齐**。
...
| 链路 | 状态 | 备注 |
|------|------|------|
| 待处理订单 | 已完成 | 读待发货/待确认订单 |
| 录入物流 | 已完成 | `clerkShipOrder` 已云函数化 |
...
- 支付为模拟支付，未接真实支付回调
- 代理商路由仍用 `salesman` 前缀
- `packages/shared` 已废弃但代码中可能有残留引用
- 当前大量改动（99 文件）未提交 git
